...
<memory-citation>
<citation_entries>
MEMORY.md:234-236|note=[model-api citation extraction code pointer]
...
The folder `{{ memory_root }}/` is a git repository managed by the agent runtime. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by the agent runtime for
...
- Retrieval bias: preserve distinctive nouns and verbatim strings that a future grep/search
  ...
  `api.example.org/v1/files`, `internal team chat`, etc.).
...
- Typical workflows and tools (how they like to work, how they use assistants and agents, preferred formats)
...
- <e.g. "When the agent added a new model-api endpoint, updating only the model-api spec left ContextAPI-generated artifacts stale. After running `<some command>` for ContextAPI as well, the generated specs matched. Future similar endpoint changes should update both surfaces.">
...
Consolidate agent memories in: {memory_root}
...
Generated by the agent runtime before Phase 2 memory consolidation. Read this file first and do not edit it.
